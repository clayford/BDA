...
- How to work with posterior distributions
...
There's about 95% probability that the mean backpack weight is between 15.2 and 16.9 lbs.

```{r echo=FALSE, fig.height=5}
plot(density(bm1.df$`(Intercept)`), main = "Posterior", 
     xlab = "possible means", xlim = c(14, 18))
abline(v = 15.17, lty=2)
abline(v = 16.96, lty=2)
text(x = 15.5, y = 0.1, labels = "P(15.2 < m < 16.9) = 0.95", pos = 4)
```
...
<!-- - Take 25 samples of each brand and calculate mean time until batteries run dead for each brand -->
...
<!-- Instead of estimating some "true" value, we instead estimate the _probability distribution_ of possible differences in means. -->

<!-- * begin with a _prior_ distribution of how we believe the difference in means are distributed -->
<!-- * update the prior distribution using the data we collected to obtain a _posterior_ distribution using _Bayes' theorem_ -->
<!-- * use the posterior distribution to describe the difference in means -->
...
- prior is how we believe a _parameter_ of interest is distributed, represented by a probability distribution. Eg: $\text{weight} \sim N(18,5)$
- likelihood is the probability of observed data _given_ a certain parameter, also represented by a probability distribution. Eg: $\text{weight}_i \sim N(\mu, \sigma)$
- posterior is the updated prior
- average likelihood ensures the posterior is a distribution with area that sums to 1
...
## Traditional approach: backpack data
...
"Gaussian" is another name for "normal". Need to specify Gaussian because a GLM can fit many different _families_ of models (eg, Poisson, Binomial, Gamma)


## Bayesian approach: backpack data
...
<!-- ## Traditional approach: battery data -->
...
<!-- $$\text{battery life}_i \sim N(\mu = \beta_0 + \beta_1\text{ grp}, \sigma)$$ -->

<!-- where $\beta_0$ is mean of group 0, $\beta_1$ is the difference in means between group 1 and group 0, and $\sigma$ is the pooled standard deviation. -->
...
<!-- ```{r} -->
<!-- t.test(y ~ grp, data = bat)$conf.int -->
<!-- ``` -->
...
<!-- ## Bayesian approach: battery data -->
...
Let's go to the R script and work some examples.
...
- Each posterior distribution will have a separate $\hat{R}$ statistic
...
- 80% posterior intervals are provided by default when calling `summary()` on a model object
- 90% posterior intervals are provided by default when using `posterior_interval()`
- According to the `posterior_interval()` documentation: "90% intervals are more stable than 95% intervals"
- You can use the `probs` or `prob` arguments to request different intervals. Say you have a model object named "m" and want 95% intervals:
    * `summary(m, probs = c(0.025, 0.975))`
    * `posterior_interval(m, prob = c(0.025, 0.975))`
...
Let's say we want to create 95% posterior intervals for the means of the two brands of batteries. The `Intercept` is the estimated mean of grp 0. The sum of the `Intercept` and `grp` coefficient is the estimated mean of grp 1.

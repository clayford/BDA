...
* The `rstanarm` package allows us to perform common Bayesian analyses using familiar base R modeling syntax
...
Effect plots are useful for main effects as well (i.e., predictors not involved in an interaction)
...
3. Calculate "performance" using a metric such as mean squared error: $\frac{1}{n}\sum_{i=1}^{n} (y_i - \hat{y_i})^2$ 

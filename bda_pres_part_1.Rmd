...
If we were to repeat this process many, many times, 95% of the confidence intervals would contain the true value. 

[^1]: If we assume backpack weights are normally distributed, then we also have to estimate the standard deviation in order to calculate the traditional confidence interval.  
...
* update the prior distribution using the data we collected to obtain a _posterior_ distribution
...
- likelihood is the probability of observed data given a certain parameter, eg. $P(16.1|\mu = 17)$
...
- Example: Multiple regression $Y = \beta_0 + \beta_1 X_1 + \beta_2 X_2 + \epsilon_i$ where $\epsilon_i \sim N(0,\sigma)$
- Need to provide prior distributions for $\beta_0$, $\beta_1$, $\beta_2$, and $\sigma$

Though this seems burdensome, it allows us to incorporate prior knowledge into our model

- Example: Home Price = $\beta_0 + \beta_1$ # bedrooms + $\beta_2$ lot size + $\epsilon_i$
- The coefficients on bedrooms and lot size will be positive; error will likely be large
- Perhaps propose a $N(20000, 2000)$ prior for $\beta_1$, a $N(10,2)$ prior for $\beta_2$, and a $\textrm{U}(10, 50000)$ for $\sigma$
...
- `rstanarm` runs 4 chains by default
...
- Included in the `summary` output of the model in the "Diagnostics" section
...
- This can often be resolved by increasing the number of iterations using the `iter` argument.
...
- The means we could have $4 \times 1000 = 4000$ samples
- The Effective sample size (ESS) corrects for any autocorrelation and approximates the number of independent samples we have
- We'd like to see ESS > 1000
- Included in the `summary` output of the model in the "Diagnostics" section
...
- PPC simulates data according to our fitted model and compares simulated to oberserved data
...
- Recall that MCSE is compared to posterior standard deviations
